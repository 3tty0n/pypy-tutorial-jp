...
トレーシング JIT は、プログラムの実行を監視し、頻繁に実行される「ホットループ」を検出してネイティブコードにコンパイルする技術です。
...
従来のメソッド単位の method JIT と異なり、tracing JIT は実際の実行パスを追跡します：

- **method JIT**: 関数全体をコンパイル
- tracing JIT**: 実際に実行されたパスのみをコンパイル
...
**効果**: JIT は同じ引数での呼び出し結果をキャッシュし、繰り返しの計算を省略できます。
